...
以下特別針對當 來源SQLServer資料庫很大(e.g. TB) 又希望能持續性的將資料異動同步到CloudSQL SQLServer, 以減少搬遷cut-over date的downtime時間.  這樣的情境下, 就推薦使用 交易式複寫 + 由備份初始化訂閱 的方式來搬遷到CloudSQL. 相惜步驟可參考此 [文件](https://docs.google.com/document/d/12CZB71dqRHWL8QoZF\_iUbp-8CmU6HaD6MjWD2-Cts-M/edit).
...
重點2: 必須用SQL指令執行, 用SSMS介面只能做用snapshot初始化訂閱,當資料庫很大的時候相當費時.  以上就是完整的步驟說明.
